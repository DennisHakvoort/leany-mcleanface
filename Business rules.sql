...
AS BEGIN
	...
	BEGIN TRY
		...
		IF (@medewerker_uren < 0) --Medewerkeruren kunnen niet negatief zijn.
			THROW 50012, 'Invalide invoerwaarde - negatieve uren', 16
		...
		IF (EXISTS(	SELECT	'!'
					...
					WHERE	p.eind_datum < CURRENT_TIMESTAMP) --project-einddatum voor huidige datum
						OR	EXISTS(	SELECT  '!'
									...
									WHERE	p.eind_datum < CURRENT_TIMESTAMP))
			THROW 50005, 'Een medewerker op een project kan niet meer aangepast worden nadat deze is afgelopen.', 16
	END TRY
	BEGIN CATCH
		THROW;
	END CATCH
		...
		*/
		IF EXISTS  (SELECT	'@'
					...
					WHERE	medewerker_code = @medewerker_code
						AND FORMAT(maand, 'yyyy-MM') = FORMAT(@maand, 'yyyy-MM')) --Datumformaten gelijkgezet en datums vergeleken.
